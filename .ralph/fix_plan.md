...
- [ ] Add residential proxy support to `src/app/api/proxy/route.ts`. Read a `RESIDENTIAL_PROXY_URL` env var (format: `http://user:pass@proxy-host:port`). If set, route all fetch calls to Rohlik through this HTTPS proxy using Node.js `undici` ProxyAgent or the native `fetch` with agent option. If not set, fall back to direct fetch (for local dev). Install `undici` if needed for proxy agent support. Test that the proxy connection works by making a single test request.
