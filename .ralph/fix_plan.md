...
- [ ] Sign up for a residential proxy service. Recommended cheap options: Webshare.io (free tier has 10 residential proxies), or BrightData, Oxylabs, SmartProxy (paid). Get a proxy URL with Czech residential IPs. Set the `RESIDENTIAL_PROXY_URL` env var in `.env` locally AND in Railway env vars via `railway variables set RESIDENTIAL_PROXY_URL=<value>`.
- [ ] Test locally with the residential proxy — set `RESIDENTIAL_PROXY_URL` in `.env`, run `npm run dev`, try to login. Verify Rohlik responds with JSON (not Cloudflare block). If it works, the full stats flow should work.
- [ ] Deploy to Railway with proxy — push changes, `railway up`. Set the `RESIDENTIAL_PROXY_URL` env var on Railway. Test the deployed app — login should now work from the Railway URL.
- [ ] Run `npm test && npm run build` to verify nothing is broken.
